/***********************************************
 * Whatalead Chatbot & WhatsApp Widget - Core Logic
 * Dynamically injects HTML, CSS and runs the widget.
 * Reads configuration from window.WhataleadChatbotConfig
 ***********************************************/
...
(function() {
    ...
    const DEFAULTS = {
        ...
        ChatbotUIConfig: {
            chatbotName: "Assistant Virtuel",
            avatarURL: null,
            helpText: "Une question ?",
            helpTextDelay: 2500,
            helpTextDisplayDuration: 6000,
            badgeContent: null,
            initialWebhookURL: null,
            messageWebhookURL: null
        },
        WhatsAppUIConfig: {
    ...
    function mergeDeep(target, source) { /* ... (même fonction que précédemment) ... */
        const output = { ...target };
            ...
            Object.keys(source).forEach(key => {
                if (isObject(source[key])) {
                    if (!(key in target)) {
                     ...
                     // On ne fusionne que si la valeur source est définie (pas undefined)
                     // Permet à l'utilisateur de définir explicitement à null ou "" pour vider une valeur par défaut
    ...
    function isObject(item) { /* ... (même fonction que précédemment) ... */
        return (item && typeof item === 'object' && !Array.isArray(item));
    }
    function loadChatHistory() { /* ... (même fonction que précédemment) ... */
        try {
            const h = sessionStorage.getItem('whatalead_chatHistory');
            return h ? JSON.parse(h) : [];
        } catch (e) {
            console.error("Erreur lecture historique chat:", e);
            return [];
        }
    }
    function saveChatHistory(history) { /* ... (même fonction que précédemment) ... */
        try {
    ...
    async function initializeChatSession(initialWebhookURL, currentConvId) { /* ... (même fonction que précédemment) ... */
        if (!initialWebhookURL) {
            console.error("Whatalead Chatbot: 'initialWebhookURL' n'est pas configuré.");
            return currentConvId || null; // Retourne l'ID existant ou null
        }
        ...
        try {
            ...
            if (!conversationId) {
                ...
                if (data && data.websiteConvId) {
                 ...
                 console.log("Session initialisée avec l'ID existant (ou confirmée par le serveur).");
            }
            return conversationId;

        } catch (error) {
            console.error("Erreur lors de l'initialisation de la session chat:", error);
            return conversationId || null;
        }
     }
    function debounce(func, wait) { /* ... (même fonction que précédemment) ... */
        let timeout;
    ...
    // --- STRUCTURES HTML & CSS --- (Injectées dynamiquement)
    const WIDGET_CSS = `
        :root { /* Default colors, will be overridden by ColorConfig */
            --bubble-gradient-start: #007BFF; --bubble-gradient-end: #00C6FF;
            --header-gradient-start: #007BFF; --header-gradient-end: #00C6FF;
            --header-text-color: #fff; --chat-bg-color: #fff; --chat-body-bg: #f7f9fc;
            --bot-bubble-bg: #e9ecef; --user-bubble-bg: #007BFF; --user-bubble-text-color: #fff;
        ...
        #chat-container { display: none; position: fixed; bottom: 95px; right: 20px; width: min(370px, 90vw); height: 520px; background: var(--chat-bg-color); border-radius: var(--chat-border-radius); box-shadow: var(--box-shadow-intense); z-index: 2147483646; flex-direction: column; overflow: hidden; transition: all var(--transition-duration) ease; opacity: 0; transform: translateY(20px) scale(0.95); }
        ...
        .bot-message, .user-message { margin: 6px 0; padding: 10px 14px; border-radius: var(--msg-border-radius); max-width: 85%; line-height: 1.45; box-shadow: 0 1px 1px rgba(0,0,0,0.05); animation: whatalead-fadeIn 0.3s ease-out; }
        @keyframes whatalead-fadeIn { from { opacity: 0; transform: translateY(8px); } to { opacity: 1; transform: translateY(0); } }
        ...
        #chat-send:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 4px 8px rgba(0, 123, 255, 0.25); }
        #chat-send:active:not(:disabled) { transform: translateY(0); box-shadow: 0 1px 2px rgba(0, 123, 255, 0.2); }
        #chat-send:disabled { opacity: 0.7; cursor: not-allowed; }
        #chat-input:disabled { background-color: #e9ecef; }
        ...
        #chat-bubble { display: none; position: fixed; bottom: 20px; right: 20px; width: 60px; height: 60px; background: linear-gradient(135deg, var(--bubble-gradient-start), var(--bubble-gradient-end)); border-radius: var(--bubble-border-radius); box-shadow: 0 5px 15px rgba(0, 123, 255, 0.3); cursor: pointer; z-index: 2147483647; justify-content: center; align-items: center; transition: all var(--transition-duration) ease-out; }
        ...
        #notification-badge { display: none; position: absolute; top: -5px; right: -5px; min-width: 22px; height: 22px; background: #DC3545; color: #fff; font-size: 12px; text-align: center; line-height: 22px; border-radius: 50%; font-weight: bold; box-shadow: 0 2px 5px rgba(0,0,0,0.2); animation: whatalead-pulse 1.5s infinite; padding: 0 5px; /* Padding for multi-digit badges */ }
        @keyframes whatalead-pulse { 0% { transform: scale(1); box-shadow: 0 2px 5px rgba(0,0,0,0.2); } 50% { transform: scale(1.1); box-shadow: 0 4px 10px rgba(0,0,0,0.3); } 100% { transform: scale(1); box-shadow: 0 2px 5px rgba(0,0,0,0.2); } }
        #help-text { display: none; position: fixed; bottom: 35px; right: 95px; background: #343a40; color: #fff; padding: 9px 15px; border-radius: 8px; font-size: 13px; z-index: 2147483645; box-shadow: var(--box-shadow-soft); animation: whatalead-fadeInHelp 0.5s ease-out; white-space: nowrap; }
        @keyframes whatalead-fadeInHelp { from { opacity: 0; transform: translateX(15px); } to { opacity: 1; transform: translateX(0); } }
        .typing-indicator { padding: 10px 14px !important; background: var(--bot-bubble-bg) !important; } /* Ensure padding */
        .typing-indicator .dot { display: inline-block; width: 6px; height: 6px; border-radius: 50%; background-color: rgba(0,0,0,0.4); margin: 0 2px; animation: whatalead-typingDots 1.2s infinite ease-in-out; }
        .typing-indicator .dot:nth-child(2) { animation-delay: 0.2s; }
        .typing-indicator .dot:nth-child(3) { animation-delay: 0.4s; }
        @keyframes whatalead-typingDots { 0%, 60%, 100% { transform: translateY(0); } 30% { transform: translateY(-4px); } }
        #whatsapp-button-container { display: none; position: fixed; bottom: 20px; right: 20px; z-index: 2147483647; flex-direction: row-reverse; align-items: center; }
        #whatsapp-text { display: none; background: #fff; color: #333; padding: 9px 14px; border-radius: 8px; font-size: 13px; margin-right: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.15); animation: whatalead-fadeInHelp 0.5s ease; white-space: nowrap; }
        ...
        #whatsapp-badge { display: none; position: absolute; top: -5px; right: -5px; min-width: 22px; height: 22px; background: #DC3545; color: #fff; font-size: 12px; text-align: center; line-height: 22px; border-radius: 50%; font-weight: bold; box-shadow: 0 2px 5px rgba(0,0,0,0.2); animation: whatalead-pulse 1.5s infinite; padding: 0 5px; }
    `;
    ...
    async function initializeWidget() {
        ...
        // 1. Lire et fusionner la configuration
        ...
        if (Config.GlobalConfig.enableChatbot && (!Config.ChatbotUIConfig.initialWebhookURL || !Config.ChatbotUIConfig.messageWebhookURL)) {
            console.error("Whatalead Chatbot: Chatbot activé mais 'initialWebhookURL' ou 'messageWebhookURL' manquant. Chatbot désactivé.");
            Config.GlobalConfig.enableChatbot = false;
        }
        if (Config.GlobalConfig.enableWhatsApp && !Config.WhatsAppUIConfig.whatsappPhoneNumber) {
            console.error("Whatalead Chatbot: WhatsApp activé mais 'whatsappPhoneNumber' manquant. WhatsApp désactivé.");
            Config.GlobalConfig.enableWhatsApp = false;
        }

        // Si rien n'est activé, ne rien faire
        if (!Config.GlobalConfig.enableChatbot && !Config.GlobalConfig.enableWhatsApp) {
             console.log("Whatalead Widget: Ni le Chatbot ni WhatsApp ne sont activés. Arrêt.");
             return;
        }

        // 3. Injecter Font Awesome (si pas déjà présent)
        ...
        // 5. Appliquer les couleurs personnalisées
        const root = document.documentElement;
        for (const [key, value] of Object.entries(Config.ColorConfig)) {
            if (value && DEFAULTS.ColorConfig.hasOwnProperty(key)) { // Applique seulement si défini et est une variable connue
        ...
        // 7. Récupérer les références DOM (MAINTENANT que le HTML existe)
        ...
        if (!chatContainer || !chatBubble || !whatsappButtonContainer) {
             console.error("Whatalead Widget: Échec de la création des éléments HTML essentiels. Arrêt.");
             // Nettoyer ce qui a été ajouté si possible
             if (widgetContainer) document.body.removeChild(widgetContainer);
             if (styleElement) document.head.removeChild(styleElement);
             // Ne pas supprimer Font Awesome au cas où il est utilisé ailleurs
             return;
        }


        // --- Variables d'état --- (identiques à avant)
        ...
        if (Config.GlobalConfig.enableChatbot && Config.GlobalConfig.enableWhatsApp) activeMode = 'both';
        else if (Config.GlobalConfig.enableChatbot) activeMode = 'chatbot_only';
        else if (Config.GlobalConfig.enableWhatsApp) activeMode = 'whatsapp_only';
        console.log("Whatalead Widget - Mode Actif:", activeMode);

        // --- Initialiser l'ID de conversation si nécessaire ---
        if (activeMode === 'chatbot_only' || activeMode === 'both') {
            ...
            if (conversationId) {
                ...
                console.warn("Whatalead Chatbot: Impossible d'obtenir un ID de session. Certaines fonctionnalités peuvent être limitées.");
            }
        }

        // --- Fonctions UI et Timers --- (Adaptées pour utiliser les variables `Config` et les éléments DOM récupérés)
        const initChatbotUI = () => {
        ...
        const initWhatsAppUI = () => { /* Rien à faire ici pour le moment, géré par visibility */ };
        const clearTimers = () => { /* ... (identique) ... */
        ...
        const runChatbotTimers = () => { /* ... (adapté pour utiliser Config.ChatbotUIConfig.* et les éléments DOM) ... */
            clearTimers();
        ...
        const runWhatsAppTimers = () => { /* ... (adapté pour utiliser Config.WhatsAppUIConfig.* et les éléments DOM) ... */
             clearTimers();
            ...
            if (Config.WhatsAppUIConfig.whatsappHelpText) {
                whatsappTimerId = setTimeout(() => {
                    ...
                    if (Config.WhatsAppUIConfig.whatsappHelpDisplayDuration > 0) {
                        whatsappHideTimerId = setTimeout(() => { whatsappTextEl.style.display = "none"; }, Config.WhatsAppUIConfig.whatsappHelpDisplayDuration);
                    }
        ...
        // --- Gestion de la Visibilité --- (identique en logique, utilise les éléments récupérés)
        const updateWidgetVisibility = () => { /* ... (identique, utilise les éléments DOM et Config) ... */
            if (isChatOpen) return;
        ...
        // --- Fonctions de Gestion du Chat --- (identiques en logique)
        const scrollToBottom = () => { /* ... (identique) ... */ setTimeout(() => { if(chatBody) chatBody.scrollTop = chatBody.scrollHeight; }, 50); };
        const appendMessage = (text, type, skipAnimation = false) => { /* ... (identique) ... */ if (!chatBody || !text) return; const d = document.createElement("div"); const c = type === 'user' ? 'user-message' : 'bot-message'; d.classList.add(c); d.textContent = text; if (skipAnimation) d.style.animation = 'none'; chatBody.appendChild(d); scrollToBottom(); };
        const showTypingIndicator = () => { /* ... (identique) ... */ if (!chatBody || chatBody.querySelector('.typing-indicator')) return; const d = document.createElement("div"); d.classList.add("bot-message", "typing-indicator"); d.innerHTML = '<span class="dot"></span><span class="dot"></span><span class="dot"></span>'; chatBody.appendChild(d); scrollToBottom(); };
        const removeTypingIndicator = () => { /* ... (identique) ... */ if (!chatBody) return; const i = chatBody.querySelector('.typing-indicator'); if (i) chatBody.removeChild(i); };
        const populateChatFromHistory = () => { /* ... (identique, ajoute message d'accueil par défaut) ... */
            if (!chatBody) return;
            chatBody.innerHTML = '';
        ...
        const sendMessage = async () => { /* ... (adapté pour utiliser Config.ChatbotUIConfig.* et vérifier conversationId) ... */
             if (!chatInput || !Config.ChatbotUIConfig.messageWebhookURL) return;
             ...
             if (!conversationId) {
                 conversationId = await initializeChatSession(Config.ChatbotUIConfig.initialWebhookURL);
                 if (!conversationId) {
                     appendMessage("Désolé, une erreur technique empêche l'envoi.", 'bot'); return;
                 }
             }
             ...
             try {
                const response = await fetch(Config.ChatbotUIConfig.messageWebhookURL, { method: "POST", headers: { "Content-Type": "application/json", "Accept": "application/json" }, body: JSON.stringify(payload) });
                ...
                console.error("Erreur msg:", error); removeTypingIndicator(); const eM = "Erreur technique."; appendMessage(eM, 'bot'); chatHistory.push({ type: 'bot', text: eM }); saveChatHistory(chatHistory);
             } finally {
        ...
        if (activeMode === 'chatbot_only' || activeMode === 'both') {
            chatBubble.addEventListener("click", () => {
                clearTimers(); helpTextEl.style.display = 'none'; notificationBadgeEl.style.display = 'none';
            ...
            chatClose.addEventListener("click", () => {
                ...
                setTimeout(() => { chatContainer.style.display = "none"; updateWidgetVisibility(); }, parseFloat(root.style.getPropertyValue('--transition-duration') || '0.3') * 1000);
            });
        ...
        if (activeMode === 'whatsapp_only' || activeMode === 'both') {
            whatsappButton.addEventListener("click", () => {
                clearTimers(); whatsappTextEl.style.display = 'none'; whatsappBadgeEl.style.display = 'none';
    ...
    // Utilise DOMContentLoaded pour s'assurer que le body existe,
    // même si le script CDN est chargé avec defer.
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initializeWidget);
    } else {
        // DOMContentLoaded a déjà eu lieu
        initializeWidget();
    }
